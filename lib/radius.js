...
// 定义请求包对象，后面会检查，如有request字符自动push增加到对象
var is_request_code = {
  "Status-Server": true
};
...
// 设置好的对象，打印出来看看
// console.log(reverse_code_map);
// console.log(is_request_code);
// console.log(uses_random_authenticator);
...
// 解码信号
// 传入参数：args = {packet: msg, secret: secret}
Radius.decode = function(args) {
  this.load_dictionaries();

  // console.log(attributes_map); // 打印出来看看

  var packet = args.packet; // 取出信号msg部分
  if (!packet || packet.length < 4) { // msg信号长度应大于等于4
    this.error("decode: packet too short");
    return;
  }

  var ret = {}; // 返回对象初始化，格式如下：
                // {
                //   code: 'Access-Request',
                //   identifer: 123,
                //   length: 250,
  ...
  ret.code = code_map[packet.readUInt8(0)]; // 解出code部分
  ...
  ret.identifier = packet.readUInt8(1); // 取出identifier和length
  ret.length = packet.readUInt16BE(2);

  if (packet.length < ret.length) { // 检查长度length是否正确
  ...
  this.no_secret = args.no_secret; // 参数传入
  this.secret = args.secret; // 参数传入，不同的调用形式，只能选择一个

  var attrs = packet.slice(AUTH_END, ret.length); // 从信号分解出attributes部分
  ret.attributes = {};
  ret.raw_attributes = [];
  ...
  // 是request类型且非Access-Request非Status-Server的包，
  if (!uses_random_authenticator[ret.code] && is_request_code[ret.code] && !args.no_secret) {
    var orig_authenticator = new Buffer.alloc(AUTH_LENGTH);
...
Radius.zero_out_message_authenticator = function(attributes) {
  ...
  for (var i = 0; i < new_attrs.length; i++) {
    ...
    if (attr[0] == ma_id) {
      new_attrs[i] = [ma_id, new Buffer.alloc(MESSAGE_AUTHENTICATOR_LENGTH)];
...
Radius.verify_response = function(args) {
  ...
  var got_checksum = new Buffer.alloc(AUTH_LENGTH);
...
// 解码信息属性部分，参数：attrs, ret.attributes, NO_VENDOR, ret.raw_attributes
Radius.decode_attributes = function(data, attr_hash, vendor, raw_attrs) {
  ...
  while (data.length > 0) {
    ...
    if (attr_info[ATTR_MODIFIERS]["encrypt=1"]) {
      ...
      case "integer":
        if (attr_info[ATTR_MODIFIERS]["has_tag"]) {
          var buf = new Buffer.from([0, 0, 0, 0]);
          value.copy(buf, 1);
          value = buf;
        }
...
Radius.encrypt_field = function(field) {
  ...
  var buf = new Buffer.alloc(len + 15 - ((15 + len) % 16));
...
Radius._crypt_field = function(field, is_decrypt) {
  ...
  for (var i = 0; i < field.length; i = i + 16) {
    ...
    var hash = new Buffer.from(hasher.digest("binary"), "binary");

    var xor_result = new Buffer.alloc(16);
    for (var j = 0; j < 16; j++) {
...
Radius.encode = function(args) {
  ...
  var packet = new Buffer.alloc(4096);
  var offset = 0;
  ...
  if (!authenticator) {
    if (uses_random_authenticator[args.code]) {
      ...
      authenticator = new Buffer.alloc(AUTH_LENGTH);
...
Radius._encode_with_authenticator = function(args, packet, offset, authenticator) {
  ...
  if (add_message_authenticator) {
    var empty_authenticator = new Buffer.alloc(MESSAGE_AUTHENTICATOR_LENGTH);
...
Radius.calculate_message_authenticator = function(packet, secret) {
  ...
  hmac.update(packet);
  return new Buffer.from(hmac.digest('binary'), 'binary');
};
...
Radius.calculate_packet_checksum = function(packet, secret) {
  ...
  return new Buffer.from(hasher.digest("binary"), "binary");
};
    ...
    if (in_value instanceof Buffer) {
      ...
      if (attr_info[ATTR_MODIFIERS]["encrypt=1"]) {
        ...
        case "text":
          ...
          out_value = new Buffer.from(in_value + "", "utf8");
          break;
        case "ipaddr":
          out_value = new Buffer.from(in_value.split("."));
